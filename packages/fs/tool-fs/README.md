...
When `ctx.fs.sandboxMode` reports confinement, write/edit advertise `sandbox_permissions` and `justification` and resolve approved retries through `ctx.approval`. A request that repeats the calling session's effective mode is idempotent and reuses the standing policy without approval; only a wider mode enters the approval path. The policy owner contributes capability-neutral standing policy; the tool results retain operation-specific denial and retry guidance.
