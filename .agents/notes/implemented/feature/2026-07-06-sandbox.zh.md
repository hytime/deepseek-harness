...
被拒绝的文件操作返回 `[sandbox: file access denied under <mode> mode]` 标记，并附带不要绕过拒绝的指令。约束执行器添加配对的 `sandbox_permissions` 与 `justification` 字段。重复请求会话有效模式时视为幂等操作，直接复用常驻策略，不要求 justification，也不触发批准；请求更宽模式时才进行一次经批准的重试，并且目标必须严格宽于会话有效模式。授权仅放宽该次重试；拒绝则不执行任何内容，返回 `the user rejected escalating this command to "<mode>"`，且不允许再次请求。由归属方派生的待处理策略上下文会说明当前文件策略，但不会取代这些强制执行边界。当 `dsh-permission-presets` 与某个 UI 适配器一起组合时，一个 preset 同时选定两个旋钮值；不匹配的组合折叠为 `custom`。[ACP 应用 bundle](../../../../packages/bundle/acp-app/README.zh.md)不挂载该 UI 服务，而是显式选定其部署模式。
...
当约束执行器被挂载时，`bash` 公布配对的 `sandbox_permissions` 和 `justification` 字段。schema 暴露完整的封闭升级词汇，因为有效模式是按会话的；重复当前有效模式的请求按幂等操作处理，其余目标必须严格宽于该调用有效模式，否则会被拒绝。真正的升级在执行前解析批准。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。
...
- 升级字段恰好在已挂载的执行器约束时存在；重复请求调用有效模式时复用该策略且不触发批准，其余不严格宽于调用有效模式的请求以自身文本失败关闭且不提示任何人；没有 ApprovalService 的部署对真正的升级调用失败关闭，对普通调用不影响。
