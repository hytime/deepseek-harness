...
- The escalation fields exist exactly when the mounted executor confines; a repeated request for the call's effective mode reuses that policy without approval, while any other target that is not strictly wider fails closed with its own text and prompts no one; a deployment with no ApprovalService fails genuine escalating calls closed and leaves plain calls untouched.
